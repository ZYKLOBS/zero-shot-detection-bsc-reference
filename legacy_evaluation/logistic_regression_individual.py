...
# Choose feature set: "D", "delta", or "both"
FEATURE_MODE = "both"
penalty ="l1"
...
plt.title(f"Precision-Recall Curve (Logistic Regression {penalty})", fontsize=16)
...
for col in feature_cols:
    if col.startswith("D"):
        if col == "D2": latex_labels.append(r"$D^{(2)}$")
        if col == "D1": latex_labels.append(r"$D^{(1)}$")
        if col == "D0": latex_labels.append(r"$D^{(0)}$")
    elif col.startswith("Δ"):
        if col == "Δ(1,2)": latex_labels.append(r"$\Delta^{(1,2)}$")
        if col == "Δ(0,1)": latex_labels.append(r"$\Delta^{(0,1)}$")
